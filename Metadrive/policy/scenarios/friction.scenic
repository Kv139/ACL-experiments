...
param weather = VerifaiOptions(['clear', 'rain', 'fog'])
param road_friction = Range(0.4, 1.0)
#ego can learn to drive slower in low friction conditions ? 
...
monitor DrivingReward(obj):
	...
	while True:
		...
		if cte < .2: 
			cte = 0
		speed_reward = max(0.5 * ego.speed, 2) * globalParameters.road_friction
